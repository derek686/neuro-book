...
<template>
    <aside class="comment-flow-panel flex h-full w-[360px] shrink-0 flex-col border-l border-[var(--border-color)] bg-[var(--editor-preview-bg)]">
        ...
        <div class="min-h-0 flex-1 overflow-y-auto p-4 custom-scrollbar">
            ...
            <div v-else class="space-y-3">
                <article
                    ...
                    class="group cursor-pointer rounded-lg border bg-[var(--bg-panel)] p-3 shadow-sm transition-all hover:-translate-y-0.5 hover:border-amber-500/55 hover:bg-[var(--bg-hover)] hover:shadow-md"
                    :class="comment.index === props.activeIndex ? 'border-amber-500 bg-amber-500/8 shadow-md ring-2 ring-amber-500/25' : 'border-[var(--border-color)]'"
                ...
                >
                    ...
                    <div v-if="editingIndex === comment.index" class="mt-3">
                        <textarea
                            v-model="editingBody"
                            rows="4"
                            class="w-full resize-y rounded-md border border-[var(--border-color)] bg-[var(--bg-input)] px-3 py-2 text-xs leading-5 text-[var(--text-main)] outline-none focus:border-amber-500"
                            @click.stop
