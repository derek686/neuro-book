...
2. 用户希望的模式：Windows Release Zip、默认“本机 + Git” `local-git`，还是高级 Docker 模式 `ghcr` / `source`。
...
NeuroBook 默认支持免 Docker 的“本机 + Git” `local-git` 部署，也保留 Docker Compose 单机部署。Windows 普通用户还可以使用 GitHub Release 中的 Windows Release Zip bootstrap 包；它独立于 `neuro-book-deploy`，首次启动会联网安装依赖、clone `master` 到 `app/` 并构建运行。运行私有配置放在项目根目录 `.env`、Boot Config `config.yaml` 和 Global Config `workspace/.nbook/config.json`，部署生成物放在 `.deploy/`。
...
这是 Windows x64 的点击启动 bootstrap 包，适合普通 Windows 本机用户。它不是 `neuro-book-deploy` 的部署模式，也不是离线包。

特点：

- GitHub Release asset，zip 自带 Node.js 24 runtime 和 root-level bootstrap 脚本。
- zip 不携带源码、`.git`、`.output` 或 `node_modules`。
- 用户运行 `Start Neuro Book.cmd` 后，脚本检查 Git、Bun、ripgrep，缺失时用 clack 交互确认并通过 `winget` 安装；`.ps1` 入口仍保留给 PowerShell 和调试场景。
- 首次启动 clone `master` 到 `app/`，在 `app/` 执行 `bun install --frozen-lockfile`、Nuxt prepare、Prisma generate、Nuxt build、SQLite migration 和管理员初始化。
- 服务 cwd 是 `app/`，Workspace Root 是 `app/workspace/`。
- 后续普通启动检查依赖、执行 migration，但不重复 build。
- 更新通过 `Update Neuro Book.cmd` / `Update Neuro Book.ps1` 跟随 `master`，用户可能进入未 release 状态；tracked worktree 脏时停止，不自动 stash 或 reset。
- root-level bootstrap 文件从 repo 内 `scripts/deploy/windows-portable/bootstrap/` 同步，Node runtime 只随新版 release zip 更新。
...
- 离线安装。
- 需要固定 release tag、不跟随 `master` 的生产环境。
...
- 服务用 `node .output/server/index.mjs` 启动。
...
- 用户明确要求免 Docker。
- 用户只安装了 npm 来启动部署脚本，但允许脚本引导安装 Git、Bun、ripgrep 等工具。
...
```bash
npx --yes --package github:notnotype/neuro-book neuro-book-deploy --deploy-mode local-git
```
...
```bash
npx --yes --package github:notnotype/neuro-book neuro-book-deploy --deploy-mode native
```
...
如果 app 不可访问，Docker 模式先看容器是否反复重启，再看 app 日志；local-git 模式直接查看运行 `node .output/server/index.mjs` 的终端输出。
