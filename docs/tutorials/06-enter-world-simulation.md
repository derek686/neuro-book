...
- `config.yaml`：世界模拟配置。
- `cast.yaml`：参与者和 profile 映射。
- `simulator.md`：simulator leader 的运行说明。
- `writer.md`：RP 输出文风和可见文本规则。
...
- `subject.md`：这个主体是谁。
- `events.md`：这个主体经历过什么。
- `knowledge.md`：这个主体知道什么。
- `mind.md`：当前心理、欲望、误解和倾向。
- `state.md`：身体、位置、持有物、关系等当前状态。

不要把上帝视角 lorebook 直接复制进 `knowledge.md`。角色只能知道它合理知道的信息。
...
4. actor 只按自己知道的信息回应。
5. simulator leader 裁决剧情推进和世界变化。
6. `rp.writer` 把裁决结果写成用户可见文本。
7. 必要状态写回 `simulation/subjects/`、`simulation/entities/` 或 `simulation/runs/`。
