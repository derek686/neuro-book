...
**校验严格度（定论：宽松但限流）**：mutation 打在 schema **未声明**的属性上是**允许**的（动态属性）。schema 是「建议结构 + 已知属性的 op 语义」，不是强约束。已知属性按声明的 kind 校验 op 是否合法、值类型是否对；未知属性默认按 `scalar` 处理，但只允许 `set` / `unset`，不允许 `add` / `listAppend` / `collectionAdd` / `collectionRemove`。这样 Agent 不必先改 schema 才能记一个新属性，同时避免打错属性名时悄悄创建错误 list / collection。
...
- schema 里属性写 `type: ref(<subjectType>)`，`<subjectType>` 用于**校验目标 subject 是否存在、type 是否匹配**与编辑期提示，不改变存储的 ref 串（存储恒为 `subject://<id>`）。第一版目标不存在或 type 不匹配直接报错；动态属性里的普通字符串不做 ref 校验。
...
- **Project SQLite 只存切片（timeline）**。切片只是 op + 值，不内嵌 schema；同一 instant 只允许一个切面，目标 instant 已有切面时 `writeSlice` 报错，调用方必须用 `editSlice` 修改已有切面。
