...
1. timeline 容器本身的数据结构（切面如何索引、排序）+ snapshot checkpoint 优化的接入点。**存储已定：schema = 项目配置文件不进 SQLite，Project SQLite 只存切片，与 plot 表共库。**
2. subject 实例的注册 / 创建（subject 身份存哪、与 schema 类型绑定；是否用「初始化切面」承载身份）。
3. re-settle 的具体触发与范围（插入 / 编辑 / 删除切面时重算哪些受影响切面的 old）。
4. 模糊时间（fuzzy / unknown）如何落到 instant INTEGER。
...
- **存储 / API（第一版最小）**：3 表（WorldSubject / WorldSlice / WorldMutation）；mutation 一行一条；instant 用 INTEGER 64 位；4 个 API（createSubject / writeSlice / getWorldState / listSlices）；snapshot、re-settle、回退、细分查询推后。详见 [sqlite-and-api.md](sqlite-and-api.md)。
