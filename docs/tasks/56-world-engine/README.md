...
- Verification surface：模型层单测（时间比较、reduce 截断、同 instant 冲突、`editSlice`、显式 re-settle）；后续接入真实 simulation workflow。
...
- **同一 `instant` 只能有一个切面**：禁止同一时间点出现多个 `WorldSlice`。同一刻发生的多 subject / 多属性变化必须位于同一个 slice 的 `mutations`。`writeSlice` 写入目标 instant 已存在时直接报错；修改已有时间点必须走 `editSlice`。reduce 排序只需要 `instant ASC` + slice 内 `mutation.seq ASC`。
- **切面结构（草图，字段后续可调）**：
...
- **一致性策略（随「已结算」必须处理的后果）**：old→new 是「应用那一刻的真实快照」，是**派生缓存**；**声明式意图（set hp = 某值）才是权威真相源**。往前插 / 编辑切面后，引擎报告 re-settle 需求，由调用方显式触发重算受影响切面的 old 值。
...
- **第一版必须提供显式 re-settle**：补过去 / 编辑切面后，写入接口先不自动改后续 `old`，而是返回 `needsResettle` 与影响范围。调用方再显式调用 `resettleTimeline(projectPath, { from, subjectIds })`，从指定 instant 开始按 subject 粗粒度重算后续 mutation 的 `old`。第一版不做 attr/path 级优化。
...
**已定方向（不再讨论）**：
- 校验**宽松但限流**：mutation 可打未声明属性（动态属性），未知属性默认 scalar，但只允许 `set` / `unset`；`add` / `listAppend` / `collectionAdd` / `collectionRemove` 必须由 schema 声明 kind 后才能使用。
...
- **存储 / API（第一版最小）**：3 表（WorldSubject / WorldSlice / WorldMutation）；mutation 一行一条；`WorldSlice.instant` 唯一；instant 用 INTEGER 64 位；API 包含 `createSubject` / `writeSlice` / `editSlice` / `resettleTimeline` / `getWorldState` / `queryState` / `listSlices`。`queryState` 是 Agent 与常规业务入口，`getWorldState` 只给 UI / 调试 / 导出使用。snapshot、回退、属性历史、反查引用推后；显式 re-settle 进第一版。详见 [sqlite-and-api.md](sqlite-and-api.md)。
...
- **ref 校验边界**：声明为 `ref(subjectType)` 的属性，目标不存在或目标 type 不匹配直接报错；动态属性里的普通字符串不做 ref 校验。
- **接入边界**：第一版不接入 `simulator.leader` / 旧 simulation workflow，只落模型、API 与 Agent 工具，待 Calendar、queryState、editSlice 和显式 re-settle 稳定后再讨论迁移。
...
- 第一版重点验证：时间比较、reduce 截断、同 instant 写入冲突、`editSlice`、显式 `resettleTimeline`、`queryState` 投影与 `listLimit`。
...
- 实现第一版 Calendar：项目可配置序列化 / 反序列化格式，parse/format 对称可逆；完整单位层级、gregorian / fantasy-simple 预设、不规则进位换算与缓存策略可后续细化。
- 定论模糊时间方案是否进第一版。
- 讨论 `editSlice` 的产品交互：编辑时是否整块替换 mutations，还是支持追加 / 删除单条 mutation。
