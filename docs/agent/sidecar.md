...
`actor.context-load` 的结果会以 `<actor_sidecar_context>` 写入 actor session。它不是一次性的隐藏注入；后续 run 和 compaction 都可以看到这条由 Harness 持久化的 actor-safe context。若注入后上下文超过模型窗口，父 invocation 会失败。
