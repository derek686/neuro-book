...
`simulator.leader` 是世界模拟 / RP 的 simulator leader。它读取 `simulation/simulator.md`、必要的 `simulation/config.yaml`、`simulation/cast.yaml`、最近 Tick、相关 lorebook 和 Plot，并调度 `simulator.actor` 与 `rp.writer`。
...
`leader.assets` 面向 user-assets 工作区，用于协助用户理解和维护 profile、Skill、writing presets、模板和覆盖层。
